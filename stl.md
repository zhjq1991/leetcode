...
class __deafult_alloc_template {
private:
...
void* refill (size_t n) {
    ...
    result = (obj*)chunk;
    my_free_list = free_list + FREELIST_INDEX(n);
    *my_free_list = next_obj = (ojb*)(chunk + n);
...
void* chunk_alloc (size_t size, int& nobjs) {
    ...
    size_t bytes_left = end_free - star_free;
...
char *chunk_alloc (size_t size, int &nobjs) {
    ...
    if (bytes_left >= total_bytes) {
        ...
        if (bytes_left > 0) {
            obj* volatile *my_free_list = my_free_list + FREELIST_INDEX (bytes_left);
            (obj*)(start_free)->free_list_link = *my_free_list;
            *my_free_list = (obj *)start_free;
        }
